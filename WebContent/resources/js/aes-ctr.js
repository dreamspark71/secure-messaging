...
/**
 * AesCtr: Counter-mode (CTR) AES Operation
 *
 * This encrypts a Unicode string to produce a base64 ciphertext using 128/192/256-bit AES,
...
class AesCtr extends Aes 
{
    ...
    static encrypt(plaintext, password, nBits) 
    {
        ...
        let key = Aes.cipher(pwBytes, Aes.keyExpansion(pwBytes)); //Produces 16-byte key
        key = key.concat(key.slice(0, nBytes-16));  // expand key to 16/24/32 bytes long


        // For initializing sub-millisec time 
        const counterBlock = new Array(blockSize);

        const nonce = (new Date()).getTime();  // timestamp: milliseconds 
        ...
        // converting sub-millisec to a string to go on the front of the ciphertext
        let ctrTxt = '';
        for (let i=0; i<8; i++) 
